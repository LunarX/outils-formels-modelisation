...
public enum Formula {
    ...
    public var dnf: Formula {
      ...
      case .disjunction(let a, let b):
        return a.dnf || b.dnf
      case .conjunction(let a, let b): // Dans ce cas on doit distribuer soit à gauche soit à droite
        switch (a, b) {
        case (_, .disjunction(let c, let d)):
          return ( (c.dnf && a.dnf).dnf || (d.dnf && a.dnf).dnf ).dnf
        case (.disjunction(let c, let d), _):
          return ( (c.dnf && b.dnf).dnf || (d.dnf && b.dnf).dnf ).dnf
        default:
          return a.dnf && b.dnf
        }
      default:
    ...
    public var cnf: Formula {
      //print("On entre dans .cnf de:", self)
        switch self.nnf {
        case .proposition(_):
          return self
        case .negation(_):
          return self
        case .disjunction(let a, let b): // Dans ce cas on doit distribuer soit à gauche soit à droite
          switch (a, b) {
